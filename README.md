# Desafio de Programação - API de Transações e Estatísticas
...
Sua missão, caso você aceite, é criar uma API REST que recebe Transações e retorna Estatísticas sobre essas transações. Para este desafio, a API deve ser criada utilizando **.NET 8** (ou superior) e **ASP.NET Core**.

Um bom lugar para se começar é utilizando a CLI do .NET (`dotnet new webapi`) ou criando um projeto de API Web através do Visual Studio / VS Code.
...
Neste desafio você deve **criar uma API REST**. **Leia com atenção todas as instruções a seguir!**

### 2.1. Restrições Técnicas

Seu projeto:

- **DEVE** estar no [GitHub](https://github.com/)
...
- Todos os commits **DEVEM** ser feitos pelo mesmo usuário que criou o projeto
  - Entendemos que algumas pessoas têm usuários pessoais e profissionais, ou um usuário diferente usado para estudar. Atenção com isso se você for uma dessas pessoas!
...
- **NÃO DEVE** utilizar quaisquer sistemas de banco de dados (como Entity Framework com SQL Server/PostgreSQL, SQLite, etc.) ou cache externo (como Redis, Memcached, etc.)
- **DEVE** armazenar todos os dados **em memória** de forma thread-safe (segura para concorrência)
- **DEVE** aceitar e responder apenas com [JSON](https://www.json.org/json-pt.html)

>**Atenção!** Por motivos de segurança, não podemos aceitar projetos enviados como arquivos compactados. Você **DEVE** disponibilizar seu projeto publicamente para que possamos acessá-lo e corrigi-lo! Após receber o feedback, sinta-se livre para tornar seu projeto **privado** :)
...
>**Dica:** Dica: O ASP.NET Core, por padrão, consegue compreender datas no padrão ISO 8601 sem problemas utilizando o System.Text.Json. Experimente utilizar um atributo do tipo `DateTimeOffset`!
...
>**Dica:** Extensões do LINQ no .NET (como `.Sum()`, `.Average()`, `.Min()`, `.Max()`, `.Count()`) podem lhe ajudar a extrair essas métricas rapidamente de forma limpa.
...
1. **Testes automatizados:** Sejam unitários e/ou integração, testes automatizados são importantes e ajudam a evitar problemas no futuro. Se você fizer testes automatizados, atente-se na efetividade dos seus testes! Por exemplo, testar apenas os "caminhos felizes" não é muito efetivo.
...
6. **Tratamento de Erros:** O .NET dá às pessoas desenvolvedoras ferramentas para se melhorar o tratamento de erros padrão. Você consegue alterar os erros padrão para retornar _quais_ erros ocorreram?
